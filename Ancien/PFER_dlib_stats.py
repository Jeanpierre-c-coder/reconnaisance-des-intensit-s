...
# initialiser le detecteur de visage de dlib (base sur HOG)
detector = dlib.get_frontal_face_detector()

# repertoire de modeles pre-formes
predictor = dlib.shape_predictor("shape_predictor_68_face_landmarks.dat")


#créer le workbook de statistiques
wb = xl.Workbook('PFER_dlib_stats_' + str(duree_app) +'.xlsx')
...
while (tps2-tps1) < duree_app:
    ...
    while (len(rects) == 0): #if face non detected
        ret, frame = cap.read()                                 #get the image
        current = cv.cvtColor(frame, cv.COLOR_BGR2GRAY)         #put it in grey
        rects = detector(current, 1)

    # Pour chaque visage detecte, recherchez le repere.
    for (i, rect) in enumerate(rects):
        # determiner les reperes du visage for the face region, then
        # convertir le repere du visage (x, y) en un array NumPy
        shape = predictor(current, rect)
        shape = face_utils.shape_to_np(shape)

    r_eyebrow = shape[17:22]
    l_eyebrow = shape[22:27]
    mouth = shape[48:68]
    ...
    shape = np.concatenate((r_eyebrow,l_eyebrow,mouth))

    kp = cv.KeyPoint_convert(shape.tolist())
    sift = cv.SIFT_create()
    kp,inputImage = sift.compute(current,kp,None)
