...
"""
Run : python PFER_stm.py
With avatar, sliding stm in the test, in real time
"""
...
expressions = ['sad', 'happy', 'surprised', 'base', 'angry']
...
sawSizeImage = 300                           #The maximum size of a SAW for the image
...
stmImage = STM(5, 1.0/nbPoints, 1.0)                         #Creation of a STM(number of neurons, norm, oblivion coef) for the visual modality

stmSliding = STM(5, 1, 0.8)                           #Creation of a STM(number of neurons, norm, oblivion coef) for sliding over emotion changes
...
while (tps2-tps1) < duree_app:
    ...
    kp, inputImage = sift.detectAndCompute(current, None)          #get points and descriptors
    #print("# kp, descriptors: {}".format(inputs_image.shape))
...
while 1 :
    ...
    output = multiplyLists(inputVector, stmSliding.neurons)
    ...
    im = plt.imread("%s.png" % expressions[np.argmax(output)])
    if img is None:
        plt.title("reconnue : %s" % expressions[np.argmax(output)])
        img = plt.imshow(im)
    else:
        plt.title("reconnue : %s" % expressions[np.argmax(output)])
        img.set_data(im)
